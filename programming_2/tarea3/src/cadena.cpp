...
TLocalizador anterior(TLocalizador loc, TCadena cad) {
    assert(localizadorEnCadena(loc, cad));
    if (loc->anterior != NULL) { return loc->anterior; }
    else { return NULL; };
}
...
bool precedeEnCadena(TLocalizador loc1, TLocalizador loc2, TCadena cad) {
    ...
    if (localizadorEnCadena(loc1, cad) && localizadorEnCadena(loc2, cad)) {
        if (loc1 == loc2) { res = true; }
        else {
            TLocalizador aux = inicioCadena(cad); //loc al inicio
            while (!res && aux != loc2) {
                if (aux == loc1) { res = true; }
                else aux = siguiente(aux, cad); //navego
            };
        };
    };
    return res;
...
TCadena insertarSegmentoDespues(TCadena sgm, TLocalizador loc, TCadena cad) {
    // si es vacia solo es igualar ini/fin
    if (esVaciaCadena(cad)) {
        cad->inicio = sgm->inicio;
        cad->final = sgm->final;
    }
    //si no, solo importa si es final
    else {
        if (!esVaciaCadena(sgm)) {
            //si es final es poner todo a lo ultimo
            if (esFinalCadena(loc, cad)) {
                //despues del final, va el primero de sgm
                loc->siguiente = sgm->inicio;
                //el final de cad pasa a ser sgm final
                cad->final = sgm->final;
                //conecto para poder navegar
    ...
    //desreferencio las puntas de sgm
...
TCadena copiarSegmento(TLocalizador desde, TLocalizador hasta, TCadena cad) {
    ...
    TCadena res = crearCadena();
    while (desde!=NULL && precedeEnCadena(desde, hasta, cad)) {
        insertarAlFinal(desde->dato, res);
    ...
    if (!esVaciaCadena(cad)) {
        if (precedeEnCadena(desde, hasta, cad)) {
            TLocalizador loc = desde;
            res->inicio = new nodoCadena;
            res->inicio->dato = loc->dato;
            res->inicio->siguiente = NULL;
            res->inicio->anterior = NULL;
            res->final = res->inicio;
            TLocalizador lresa = res->inicio;
            loc = loc->siguiente;
            while (precedeEnCadena(loc, hasta, cad) && loc != NULL) {
                TLocalizador lres = lresa->siguiente;
                lres = new nodoCadena;
                lres->dato = loc->dato;
                lres->anterior = lresa;
                lresa->siguiente = lres;
                lresa = lres;
                loc = loc->siguiente;
            };
            loc = NULL;
...
TCadena borrarSegmento(TLocalizador desde, TLocalizador hasta, TCadena cad) {
    if (!esVaciaCadena(cad) && precedeEnCadena(desde, hasta, cad)) {
        TLocalizador borrador = desde;//borra
        TLocalizador loc = desde;//donde borra
        while (precedeEnCadena(loc, hasta, cad)) {
            loc = loc->siguiente;
            removerDeCadena(borrador, cad);
            borrador = loc;
        };
    };
    return cad;
...
TLocalizador anteriorClave(nat clave, TLocalizador loc, TCadena cad) { //CORE DUMPED ----a ver
    if (!localizadorEnCadena(loc, cad)) { return NULL; }
    else {
        while ((loc!=NULL) && (clave != (natInfo(loc->dato)))) {
                loc = loc->anterior;
            };
