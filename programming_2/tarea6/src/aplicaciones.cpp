...
bool auxesAvl(int &a, TAbb abb){
  if(esVacioAbb(abb)){
    ...
    if (izquierdo(abb) == NULL && derecho(abb) == NULL)
    {
      ...
      int der = a;
      if(auxesAvl(a, izquierdo(abb)) && auxesAvl(der, derecho(abb))){
        int diff = abs(a-der);
        if(der>a){
          a = der;
        }
...
bool esAvl(TAbb abb) {
  if(esVacioAbb(abb)){
    ...
    int ai = alturaAbb(izquierdo(abb));
    int ad = alturaAbb(derecho(abb));
    int res = abs(ai-ad);
    return (res <= 1) && (esAvl(izquierdo(abb)) && esAvl(derecho(abb)));
                }*/
                int a = 1;
    return auxesAvl(a, abb);
  }
}
